...
- [...`redux-logic` or `reselect`](https://github.com/mxstbr/react-boilerplate/blob/master/docs/js/remove.md).
...
If your application is going to interact with some back-end application for data, we recommend using redux logic for side effect management. Too much jargon? Let's simplify.
...
The mental model is that a logic is like a separate thread in your application that's solely responsible for side effects. `redux-logic` is a redux middleware, which means this thread can be started, paused and cancelled from the main application with normal redux actions, it has access to the full redux application state and it can dispatch redux actions as well.
...
Check out [`HomePage/sagas.js`](https://github.com/mxstbr/react-boilerplate/blob/master/app/containers/HomePage/sagas.js). It can be confusing for untrained eyes. The API of `redux-saga` is self-descriptive once you've seen it, so let's go over what happens in there:
